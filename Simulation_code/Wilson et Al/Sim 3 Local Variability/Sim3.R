...
plot_simulation_summary(dynamic_networks, Z_list, node_labels, sim_title = "Simulation 3")

# === Compute F1–F9 summaries ===

F_time_series <- compute_and_plot_F(dynamic_networks, plot=TRUE, sim_title='Simulation 3')
par(mfrow = c(1, 1))

lad_results <- run_lad_analysis(
  adjacency_list = dynamic_networks,
  changepoints = t_star,  # Known structural change
  title = "LAD - Simulation 3",
